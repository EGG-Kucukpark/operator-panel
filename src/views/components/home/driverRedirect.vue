<template>
    <b-card>
        ...
        <b-row>
            <div style="margin-left:12px;  align-items:center" class="row">
                <h1>Otopilot</h1>
                <b-form-checkbox v-model="autopilot" @change="autopilotChanged" style="margin-left:10px;"
        ...
        <b-form @submit.prevent="driverRedirect">
            <b-card-text class="row">
                <b-form-group class="col-lg-4 col-sm-12" label="Sürücü Numarası" label-for="surucu">
                    ...
                    <v-select :disabled="autopilot || isApptoday" v-model="selectedDriver" :filterable="true"
                        label="name" :options="drivers" class="style-chooser"
                        :selectable="(option) => option.status == 'online'" placeholder="Lütfen Sürücü Seçiniz ">
                        <template slot="no-options">Sonuç yok.</template>

                        <template v-if="option.status == 'online'" slot="option" slot-scope="option">{{
                            option.name
                        }}</template>
                ...
                <b-form-group label="Müşteri Numarası" label-for="musteri" class="col-lg-4 col-sm-12">
                    <v-select v-model="selectedCustomer" :filterable="true" :disabled="autopilot" label="userPhone"
                        taggable :options="customers" class="style-chooser"
                        ...
                        <li slot="list-footer" class="pagination">
                            <b-button variant="flat-info">Güncel
                                Müşteriler</b-button>
 ...
 -->
                ...
                <b-form-group label="Müşteri Notu" label-for="musteri" class="col-lg-4 col-sm-12">
                    <b-form-input :disabled="autopilot" v-model="note"
                ...
                <b-form-group style="align-items:center; display:flex; justify-content:center" class="col-12">
                    <b-button :disabled="autopilot" style="margin-top:23px;" variant="success" type="submit">
                        Sürücüye Yönlendir
                    </b-button>
                </b-form-group>
            </b-card-text>
