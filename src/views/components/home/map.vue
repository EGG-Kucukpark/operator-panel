<template>
  <b-card no-body>
    ...
    >
      <GmapMarker v-for="driver in drivers" :key="driver.id" :position="setDriverLocation(driver)" :icon="setIcon(driver)" @click="setDriverWindowData(driver)" />

      <GmapMarker
        v-for="customer in customers"
        :key="customer.id"
        :position="setCustomerLocation(customer)"
        icon="../public/../assets/customer.png"
        @click="setCustomerWindowData(customer)"
      />

      <GmapMarker v-for="destination in destinations" :key="destination.id" :position="setDestinationLocation(destination)" @click="setDestinationWindowData(m)" />

      <gmap-info-window :opened="isDriverWindowOpen" :position="driverWindowLocation" class="infowindow" @closeclick="isDriverWindowOpen = false">
        <div class="infowindow" style="display: flex; flex-direction: column">
          ...
          <b-badge :variant="setVariant(driverWindowData.status)" style="height: 30px">
            <h4 style="color: white; text-align: center; text-transform: capitalize">{{ driverWindowData.status }}</h4>
          </b-badge>
        </div>
      </gmap-info-window>

      <gmap-info-window :opened="isCustomerWindowOpen" :position="customerWindowLocation" @closeclick="isCustomerWindowOpen = false">
        <div class="infowindow" style="display: flex; flex-direction: column">
          ...
          <p style="font-weight: 500">
            ...
            <span v-if="!isLoading && customerWindowData.driver">{{ customerWindowData.driver.name + "/" + customerWindowData.driver.distanceText }} </span>

            <span v-if="!isLoading && customerWindowData.driver === null"> Araç Bulunamadı </span>
          </p>
          ...
          <p v-if="customerWindowData.location" style="font-weight: 500">
            <a target="_blank" :href="'https://maps.google.com/?q=' + customerWindowData.location.degreesLatitude + ',' + customerWindowData.location.degreesLongitude"
              >Haritada Göster</a
            >
          </p>

          <b-button v-if="customerWindowData.driver != null" variant="success" @click="driverRedirect(customerWindowData.driver, customerWindowData.userPhone)"
            >{{ customerWindowData.driver.name }} Taksi Yönlendir</b-button
          >
        </div>
      </gmap-info-window>

      <gmap-info-window :opened="isDestinationWindowOpen" :position="destinationWindowLocation" @closeclick="isDestinationWindowOpen = false">
        <div class="infowindow" style="display: flex; flex-direction: column">
          ...
          <p style="font-weight: 500">
            <b-spinner v-if="!destinationWindowData.distane && !destinationWindowData.time" small variant="second" />
            <span v-else>
...
export default {
  ...
  methods: {
    ...
    async calculateDestination(data) {
      ...
      let finalData = (await axios.post("https://www.turkpark.com.tr:2222/driverDestination", { destination: data.destination, driver: { lat: tripData.lat, lng: tripData.lng } }))
        .data[0];
      return finalData;
    },
