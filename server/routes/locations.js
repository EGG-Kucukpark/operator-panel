...
const locationSchema = require('../models/location/locationDB');
...
router.post('/calcLoc', async (req, res) => {
    ...
    for (let i = 0; i < drivers.length; i++) {
            ...
            await axios(`https://maps.googleapis.com/maps/api/distancematrix/json?origins=${customer.degreesLatitude}%2C${customer.degreesLongitude}&destinations=${drivers[i].lat}%2C${drivers[i].lng}%&key=AIzaSyBA6gH1LTs7-6RTKi__xXvdaZ4bV78IdOE`).then((item) => {
                let driver = {
                    id: drivers[i].id,
                    name: drivers[i].name,
